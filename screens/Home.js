...
import { View, StyleSheet, TextInput, FlatList, Image, TouchableOpacity, ScrollView } from 'react-native'
import { SafeAreaView } from 'react-native-safe-area-context'
import PlanetHeader from '../src/components/planetHeader'
...
export default function Home({ navigation }) {
    const [itemList, setItemList] = useState(RECIPES)

    const renderItem = ({ item, index }) => {
        const { name, imageURL, ingredients, steps } = item;
        return (
            <TouchableOpacity onPress={() => navigation.navigate('Details', { planet: item })} style={style.itemList} >
                <Image
                    style={style.image}
                    source={{
                        uri: imageURL,
                ...
                <View style={style.itemBody}>
                    <Text preset="h6 bold colorBlack pb">{name}</Text>

                    <FlatList
                        data={ingredients}
                        renderItem={ingredientItem}
                        keyExtractor={(iNitem, index) => iNitem.name}
                    // contentContainerStyle={style.itemBody}
                    />

                    <Text preset="h6 semiBold colorBlack pb pt">Step by Step guidlines</Text>

                    {steps.map((item, key) => (
                        <View>
                            <Text preset="colorBlack pbSm">{key + 1} - {item}</Text>
                        </View>
                    )
                    )}
    ...
    const seachRecipeName = (text) => {
        const recipeList = RECIPES.filter((Ritem) => {
            const Rname = Ritem.name.toUpperCase();
            const typeText = text.toUpperCase();
            return Rname.indexOf(typeText) > -1;
        })
        setItemList(recipeList);
    }
    const ingredientItem = (iNitem, index) => {
        return (
            <Text preset="colorBlack pbSm">{iNitem.index + 1} - {iNitem.item.quantity} {iNitem.item.name}</Text>
        )
    }
    ...
    return (
        <View style={style.body}>
            <TextInput
                style={style.input}
                onChangeText={(text) => seachRecipeName(text)}
